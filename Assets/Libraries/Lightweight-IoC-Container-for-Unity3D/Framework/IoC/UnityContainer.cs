...
namespace IoC
{
	public class UnityContainer:Container
	{
		...
		{
			#if NETFX_CORE
			// http://forum.unity3d.com/threads/windows-store-apps-and-reflection-code.253666/
			if (typeof(MonoBehaviour).GetTypeInfo().IsAssignableFrom(mapper.GetTypeInfo()) == false)				
			{
		...
		override public void Register(System.Type type)
		{
			#if NETFX_CORE
			// http://forum.unity3d.com/threads/windows-store-apps-and-reflection-code.253666/
			if (typeof(MonoBehaviour).GetTypeInfo().IsAssignableFrom(type.GetTypeInfo()) == false)				
			{
				base.Register(type);
			}
			#else
